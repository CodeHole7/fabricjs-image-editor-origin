(function () {
  ...
  var toolbar = function () {
    const _self = this;
    try {
      this.containerEl.append(`<div class="toolbar" id="toolbar"><div class="main-buttons"></div><div class="zoom-levels"></div><div class="extended-buttons"></div></div>`);
      ...
      // zoom
      (() => {
        $(`${this.containerSelector} #toolbar .zoom-levels`).append(`
          <label>Zoom</label>
        ...
        `);
        $(`${this.containerSelector} #toolbar .zoom-levels #input-zoom-level`).change(function () {
          let zoom = parseFloat($(this).val());
          _self.canvas.setZoom(zoom)
          _self.canvas.setWidth(_self.canvas.originalW * _self.canvas.getZoom())
          _self.canvas.setHeight(_self.canvas.originalH * _self.canvas.getZoom())
        })
      })();
      // extended buttons
      (() => {
        ...
        $(`${this.containerSelector} #toolbar .extended-buttons button`).click(function () {
          ...
          if (id === 'save') {
            ...
            $('body').append(`<div class="download-modal-container">
            ...
            $(".download-modal-container").click(function () {
              $(this).remove();
            })

            $(".download-modal-container .button-download").click(function (e) {
            ...
            try {
              let undoList = _self.history.getValues().undo;
              if (undoList.length) {
                let current = undoList[undoList.length - 1];
                _self.history.undo();
                current && _self.canvas.loadFromJSON(JSON.parse(current), _self.canvas.renderAll.bind(_self.canvas))
              }
            } catch (_) {}
          } else if (id === 'redo') {
            try {
              let redoList = _self.history.getValues().redo;
              if (redoList.length) {
