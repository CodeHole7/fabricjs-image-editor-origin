(function () {
  ...
  var selectionSettings = function () {
    const _self = this;
    $(`${this.containerSelector} .main-panel`).append(`<div class="toolpanel" id="select-panel"><p class="title">Selection Settings</p></div>`);

    // font section
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel`).append(`
    ...
    (() => {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .fill-section #color-picker`).change(function () {
        let enabled = $(`${_self.containerSelector} .toolpanel#select-panel .fill-section #color-fill`).is(':checked')
        if (!enabled) return;
        let color = $(this).val();
      ...
      gp.on('change', complete => {
        let enabled = $(`${_self.containerSelector} .toolpanel#select-panel .fill-section #gradient-fill`).is(':checked')
        if (!enabled) return;
        let gradient = generateFabricGradientFromColorStops(gp.getHandlers(), _self.activeSelection.width, _self.activeSelection.height);
